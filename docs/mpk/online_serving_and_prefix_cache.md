...
> 本文是**给人读的设计文档**；配套的[复现 runbook（给 agent）](oipl_repro.md)、[benchmark 数据](oipl_benchmarks.md)与[总览 README](oipl_README.md)。
...
- **(c′) 背压必须页归因（P2 硬件验证的修正）**：原 (c) 规则"最老发布请求久等未准入 → 强制驱逐"在**批满（床位耗尽）**时误触发——实测 4 床位挂 12 在飞请求时 0.8s 内空烧 1061 次背压、在 47–56/64 页空闲的情况下驱逐了 163 个块、把缓存剥到只剩被 pin 的链。修正：强制驱逐需同时满足 (i) 最老发布请求 stall 超阈值，**(ii) 页归因成立：free 估计（free-count 镜像 + 台账）< 头请求的 worst_need**。床位饥饿不驱逐——驱逐解决不了它，只会白白清空缓存。**两个输入必须同向宽松**：free 估计（free-count 镜像 + 归还在途）是 `avail_uncommitted` 的**上界**（它看不见已准入行的预留，仿真器 I9 逐步断言这一点），所以 worst_need 也取上界 `ceil(MAX_SEQ/PS)`——即使 CPU 自己发布过头请求的 npp、能算出更紧的 `pages_per_req − npp` 也不减：少触发一次是准入停摆，多触发一次只多还一个块。背书：`scenario_row_starvation`（床位耗尽、页充裕 → 强制驱逐必须为 0；注入 `unattributed_backpressure` 即复现空烧）与 `scenario_page_starvation`（页真短 → 必须驱逐；注入 `no_backpressure` 即 LivenessViolation）。
...
| 多轮对话（A3） | ✅**已落地 2026-09-01**（`53ea03fc`）：messages 全量过模板、仅在客户端未给 system 时注入默认、`/v1/completions` 改真续写。实测 5 轮对话逐轮 npp 增长 [0,4,6,7,9]，TTFT 缓存开时随轮**下降**（0.070→0.024s）vs 关时随 prompt 长度上升（0.067→0.159s），第 5 轮对未缓存 6.5×。单轮路径 token 级字节等价（唯一有意变化：客户端自带 system 不再被覆盖） |
...
| **P2** ✅已落地 2026-09-01 | 缓存 + 命中（prompt-only）：所有者线程、发布时匹配、去重插入。**实测：2000-token 共享前缀 TTFT 608.65ms→24.55ms（24.8×）**，9/9 出口检查绿 | hit rate、TTFT p50/p99、迭代时间无回归；**模板前缀稳定性单测在此定论** |
| **P3** | 驱逐 + 水位启发式 + 压力（池占满/驱逐风暴/所有者线程 kill 注入）+ v2.1-a/c′ 两条活性规则。**(c′) 页归因背压已落地**：仿真器加 free-count 镜像通道 + 不变量 I9（CPU free 估计恒为 `avail_uncommitted` 上界）与 `row-starvation`（零强制驱逐）/`page-starvation`（必须驱逐）两个场景，`prefix_cache.tick()` 收下 `free_estimate`/`head_worst_need` 并计 `backpressure_suppressed`，注入 `unattributed_backpressure` 可复现 P2 的空烧 | **双边验收**：GPU 准入门触发频率 ≈ 0（否则池配小了），且无已发布请求等待超过 X 个管家周期（否则活性规则失效） |
