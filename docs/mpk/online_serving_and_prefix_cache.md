...
- **(c′) 背压必须页归因（P2 硬件验证的修正）**：原 (c) 规则"最老发布请求久等未准入 → 强制驱逐"在**批满（床位耗尽）**时误触发——实测 4 床位挂 12 在飞请求时 0.8s 内空烧 1061 次背压、在 47–56/64 页空闲的情况下驱逐了 163 个块、把缓存剥到只剩被 pin 的链。修正：强制驱逐需同时满足 (i) 最老发布请求 stall 超阈值，**(ii) 页归因成立：free 估计（free-count 镜像 + 台账）< 头请求的 worst_need**。床位饥饿不驱逐——驱逐解决不了它，只会白白清空缓存。
